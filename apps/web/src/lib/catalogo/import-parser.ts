...
function csvRowToProductoInput(row: CsvRow) {
  return {
    ...
    precioCents: parseNumberCell(getRowValue(row, ["precioPesos", "precioCents", "price_cents", "precio_centavos", "precio_pesos"])),
...
function coerceJsonItem(item: unknown) {
  ...
  return {
    ...
    precioCents: candidate.precioPesos ?? candidate.precioCents ?? candidate.price_cents,

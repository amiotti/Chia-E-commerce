...
export async function readAdminProductos(): Promise<Producto[]> {
  ...
  if (error) {
    throw new Error(`Error leyendo productos desde Supabase: ${error.message}`);
  }

  const parsed = z.array(supabaseProductRowSchema).safeParse(data ?? []);
  if (!parsed.success) {
    throw new Error("La tabla products devolvió filas incompatibles con el schema esperado.");
  }
...
export async function createManualAdminProducto(input: unknown): Promise<Producto> {
  ...
  if (error) {
    throw new Error(`No se pudo guardar el producto en Supabase: ${error.message}`);
  }
...
export async function importAdminProductos(items: Producto[], mode: ImportMode) {
  ...
  if (mode === "replace") {
    ...
    if (idsError) {
      throw new Error(`No se pudieron leer IDs de products para replace: ${idsError.message}`);
    }
    ...
    if (idsToDelete.length > 0) {
      ...
      if (deleteError) {
        throw new Error(`No se pudieron borrar productos administrados en replace: ${deleteError.message}`);
      }
    }
  }
  ...
  if (items.length > 0) {
    ...
    if (upsertError) {
      throw new Error(`No se pudieron importar productos a Supabase: ${upsertError.message}`);
    }
  }

...
const supabaseProductRowSchema = z.object({ id: z.string(), slug: z.string(), name: z.string(), description: z.string(), price_cents: z.number().int().nonnegative(), currency: z.string(), images: z.array(z.string().url()).nullish().transform((value) => value ?? []), stock: z.number().int().nonnegative(), category: z.string(), tags: z.array(z.string()).nullish().transform((value) => value ?? []), active: z.boolean(), points_enabled: z.boolean().default(false), points_cost: z.number().int().positive().nullable().optional() });

function createAdminProductId() { return `adm_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }

function toSupabaseProductRow(producto: Producto) {
  return { id: producto.id, slug: producto.slug, name: producto.nombre, description: producto.descripcion, price_cents: producto.precioCents, currency: producto.moneda, images: producto.imagenes, stock: producto.stock, category: producto.categoria, tags: producto.tags, active: producto.activo, points_enabled: producto.canjeConPuntos, points_cost: producto.canjeConPuntos ? producto.puntosCanje : null };
}
...
function isManagedAdminProductId(id: string) { return id.startsWith("adm_") || id.startsWith("imp_"); }

export async function readAdminProductos(): Promise<Producto[]> {
  const client = requireSupabaseServiceClient() as any;
  const { data, error } = await client.from("products").select("id, slug, name, description, price_cents, currency, images, stock, category, tags, active, points_enabled, points_cost").order("slug", { ascending: true });
...
export async function createManualAdminProducto(input: unknown): Promise<Producto> {
  ...
  const client = requireSupabaseServiceClient() as any;
  const { data, error } = await client.from("products").upsert(toSupabaseProductRow(producto), { onConflict: "slug" }).select("id, slug, name, description, price_cents, currency, images, stock, category, tags, active, points_enabled, points_cost").single();
...
export async function importAdminProductos(items: Producto[], mode: ImportMode) {
  ...
  if (mode === "replace") {
    ...
    const idsToDelete = (idsData ?? []).map((row: unknown) => String((row as { id?: string }).id ?? "")).filter((id: string) => id && isManagedAdminProductId(id));

...
export default async function LoginPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return (
    <CuentaAuthShell title="Ingresar" subtitle="Accedé a tu cuenta. Si todavía no tenés usuario, podés registrarte en el siguiente paso.">
      <h2 className="font-brand text-3xl text-[#0B3816]">Login</h2>
      <p className="mt-2 text-sm text-[#0B3816]/75">Protege el panel admin y futuras acciones de carrito/órdenes.</p>
      ...
      <p className="mt-4 text-sm text-[#0B3816]/75">
        ¿No tenés cuenta?{" "}
        <Link href={`/cuenta/registro?redirectTo=${encodeURIComponent(redirectTo)}`} className="text-[#587055] underline underline-offset-4">
          Crear cuenta
        </Link>
      </p>
    </CuentaAuthShell>

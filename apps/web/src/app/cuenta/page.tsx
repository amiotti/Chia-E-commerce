...
export default async function CuentaPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const orders = isAdmin ? await listAllOrders() : await listOrdersByUser(session.userId);
      ...
      <div className="relative mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <section className="panel-surface rounded-3xl border border-[#587055]/15 p-6 sm:p-7">
          ...
          <p className="mt-3 text-sm text-[#0B3816]/75 sm:text-base">
            {isAdmin
              ? "Desde tu cuenta podés revisar datos, administrar productos y controlar manualmente el estado de pago de las órdenes."
              : "Desde tu cuenta podés revisar tus datos, seguir tus pedidos y acceder rápidamente a tus compras."}
          </p>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {isAdmin ? (
              <>
                ...
                <Link href="/cuenta/ordenes" className="rounded-full border border-[#587055]/25 bg-white/80 px-4 py-2 text-sm text-[#0B3816] hover:bg-[#F0ECDF]">Gestionar órdenes</Link>
              </>
            ) : (
          ...
          {isAdmin ? (
            <div className="mt-6 rounded-3xl border border-[#587055]/10 bg-white/60 p-5">
              <div className="mb-4">
                <p className="text-xs uppercase tracking-[0.2em] text-[#587055]">Gestión manual</p>
                <h2 className="font-brand text-3xl text-[#0B3816]">Todas las órdenes</h2>
                <p className="mt-2 text-sm text-[#0B3816]/75">
                  Desde esta sección podés marcar manualmente una orden como pagada cuando hayas confirmado la transferencia.
                </p>
              </div>
              <OrdersPanel initialOrders={orders} isAdmin />
            </div>
          ) : (
            <div className="mt-6 rounded-3xl border border-[#587055]/10 bg-white/60 p-5">
              <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <h2 className="font-brand text-3xl text-[#0B3816]">Órdenes recientes</h2>
                </div>
              ...
              {orders.length === 0 ? (
                <div className="rounded-2xl border border-[#587055]/10 bg-white/70 p-5 text-sm text-[#0B3816]/75">
                  Todavía no tenés órdenes. <Link href="/catalogo" className="text-[#587055] underline underline-offset-4">Ir al catálogo</Link>
                </div>
              ) : (
          ...
          <form action="/api/auth/logout" method="post" className="mt-6">
            ...
            <button type="submit" className="rounded-2xl border border-[#B8858E]/35 bg-[#B8858E]/10 px-4 py-2.5 text-sm font-medium text-[#0B3816] hover:bg-[#B8858E]/20">Cerrar sesión</button>
          </form>
        </section>
      </div>
    </main>

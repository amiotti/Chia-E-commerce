    ...
    <main className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        <header className="panel-surface mb-5 rounded-3xl border border-[#587055]/15 p-5 sm:p-6">
          ...
          <p className="mt-2 text-sm text-[#0B3816]/75">Historial local de órdenes creadas en el Paso 4.</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <Link href="/cuenta" className="rounded-full border border-[#8BA37D]/35 bg-[#F0ECDF] px-4 py-2 text-sm text-[#0B3816] hover:bg-white">
              Mi cuenta
            </Link>
                ...
                <article key={order.id} className="rounded-2xl border border-[#587055]/10 bg-white/70 p-4">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-semibold text-[#0B3816]">{arsFormatter.format(order.totalCents / 100)}</span>
                      <Link href={`/ordenes/${order.id}`} className="rounded-xl bg-[#0B3816] px-3 py-2 text-sm text-[#F0ECDF] hover:bg-[#587055]">
                        Ver detalle
                      </Link>
                    </div>
                  </div>
